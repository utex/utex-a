...
#if defined(NDEBUG)
# error "Utex cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Utex Signed Message:\n";
...
{
    // Utex: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Utex: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Utex: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Utex: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UtexMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UtexMiner : ProcessBlock, block not accepted");
    }
...
void static UtexMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running UtexMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UtexMiner, pwallet));
}
